...
def on_message(client, userdata, msg):
  ...
  try:
      ...
      insertline = """INSERT INTO projintegrador (id, origem, descricao, valor, pub_date, pub_time) VALUES (%s, %s, %s, %s, %s, %s);"""

      w_sensor_id = metrica
      w_fonte = fonte
      w_caracterizacao = caracterizacao
      w_sensor_value = sensor
      w_pub_date = data_e_hora_em_texto
      w_pub_time = datetime.datetime.now().timestamp()

      values = (w_sensor_id, w_fonte, w_caracterizacao, w_sensor_value, w_pub_date, w_pub_time)
